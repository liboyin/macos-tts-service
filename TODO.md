...
These initial full boundaries are referenced from the backlog rather than duplicated there. Suggested order: NB19; B2 and NB9; response/credential work; settings/catalog simplification. This is priority guidance, not a phase fence. Every change needs its assigned scope.

### NB19 execution boundary

**Intent:** implement D1's shared owner and the ordered terminal interface B2 needs, preserving entry-point product policy. **Dependencies:** build on the landed B1 test-ownership contract and re-read the landed B3 pause-revocation and B4 progress-tick code; introduce the owner before B2.
